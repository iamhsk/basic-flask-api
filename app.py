...
def register():
    ...
    if test:
        ...
        user = User(first_name=first, last_name=last, email=email, password=pwd)
        db.session.add(user)
        db.session.commit()
...
def login():
    ...
    if test:
        ...
        return jsonify(message='Bad email or password'), 201
...
def remove_planet(planet_id: int):
    ...
    if planet:
        ...
        return jsonify(message="That planet does not exist"), 202
...
class PlanetSchema(ma.Schema):
    class Meta:
        fields = ('planet_id', 'planet_type', 'home_star', 'mass', 'radius', 'distance')
